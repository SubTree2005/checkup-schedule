...
Checkup Schedule 是体检检查智能排序项目的 Monorepo。当前项目发布版本为 **0.3.0**，其中 Scheduler 算法为 **V10（等待优先版）**：它保留硬时间窗、前置关系、Rolling Horizon、等待预测与双反馈边界，以预计等待、未来拥堵、长时间停滞、步行和路线稳定性为主要软目标。
