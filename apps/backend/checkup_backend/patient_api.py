...
from .navigation_routes import route_via
from .navigation_gis import prepare_navigation_floors, guidance_waypoints
...
def patient_navigation(
    ...
    if map_data and map_data.get("guidanceNotice"):
        floor_instruction = map_data["guidanceNotice"] + " " + floor_instruction
    if map_data and map_data.get("routeUnavailableNotice"):
        floor_instruction = map_data["routeUnavailableNotice"]
        distance = None
...
def _navigation_map(
    ...
    floors = prepare_navigation_floors(floors)
    locations = {}
    ...
    waypoints = guidance_waypoints(floors, target_props, to_department_id)
    if source:
        routed = route_via(floors, source, (*target_location, to_name), waypoints)
        if routed:
    ...
    if from_location:
        ...
        if source_floor.floor_key != target_floor.floor_key:
            from_point = None
        if (waypoints == [] and source_floor.floor_key == target_floor.floor_key
    ...
    return {
        **({"routeUnavailableNotice": "完整路线暂不可用，请按现场指引前往导诊点和目标科室。"} if waypoints is None or waypoints else {}),
